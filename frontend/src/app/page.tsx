...
export default function Home() {
  ...
  const fetchEvents = async (cityName: string) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`https://louder.onrender.com/api/v1/fetch-events/${cityName}`);
  ...
  const saveUserAndRedirect = async () => {
    ...
    try {
      const response = await fetch("https://louder.onrender.com/api/v1/save-user", {
        method: "POST",
        headers: {
